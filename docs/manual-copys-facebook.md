...
sequenceDiagram
    autonumber
    actor M as Maestro en Grupo FB
    actor G as Growth Marketer (Dezzpo)
    participant FB as Grupo de Construcción
    participant DM as Chat Privado / Sub-comentario

    G->>FB: Publica Copy SIN_URL con palabra clave ("YO", "PÁGINA", "FOTOS")
    M->>FB: Lee gancho y comenta: "YO" / "PÁGINA"
    Note over FB: El algoritmo de FB detecta alto engagement e impulsa el post
    G->>DM: Responde en <5 min con link trackeado (dezzpo.com/?utm_source=fb...)
    M->>DM: Da clic, crea su perfil gratis en 2 minutos
```
